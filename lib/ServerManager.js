...
//add a remote client
ServerManager.prototype.addConnection = function(socket, info, next) {
	var self = this;
...
ServerManager.prototype.childrenDisplay = function(params) {
	if (params.count) return this.count;
	var self = this;
	var result = [];
	self.filter(function (serverEnd) {
		result.push(serverEnd.display(params));
	});
	return result;
};

ServerManager.prototype.display = function(params) {
	return {
		servers : this.childrenDisplay(params),
